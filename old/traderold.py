...
def getLoopParams(token, timeframe, pair):
    df = pd.read_sql(f"SELECT token, pair, timeframe, percentage_of_available FROM main.parameters group by token, pair, timeframe, percentage_of_available",con=db_con)
    return df 
...
def trader(values, env, token, timeframe, pair):
    while True:
        print('Looping')
        params = getLoopParams(env, token, timeframe, pair)
            ...
            if  check_params("main", row.token, row.pair, row.timeframe) == False:
                ...
                if signal['status'][0] == 0:
                    ...
                    if operations['counterBuy'][0] == 0:
                        ...
                        ForceSell = float(params['stoploss'][0] / 100) # %
                        #
                        #
